...
import('engine.core.CachedObjectContainer');
...
class EntityCacher {
	...
	/** Verifica se a instancia esta em cache
	 * 
	 * @param object $object
	 */
	public function contains($object) {
	    return $this->lookup(EntityManager::getCacher()->getOIDString(($object instanceof IEntityContainer ? $object->getSubject() : $object))) ? true : false;
	}
	...
	public function refreshState($oid) {
		if (isset($this->data[$oid])) {
			$this->data[$oid]->refreshState();
		} else {
			throw new Exception("Item nao encontrado em cache (".$oid.").");
		}
	}
	...
	public static function makeOIDString($entity) {
		$entity = ($entity instanceof IEntityContainer) ? $entity->getSubject() : $entity ;
		$classMeta = EntityManager::getReflectionData(get_class($entity));
		$oidName = $classMeta->getName();
		if($classMeta->getParentClass()) {
			$classMeta = $classMeta->getParentORMClass();
		...
		foreach ($classMeta->getIndexes() as $column) {
			$idxValue = $column->getValue($entity);
			if (empty($idxValue)) return false;
			if ($column->isForeignKey()) {
				foreach ($column->getForeignIndexORMProperties() as $currentIndex) {
					$oidName .= "#".$currentIndex->getValue($entity);
				}
