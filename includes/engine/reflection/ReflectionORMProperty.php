...
class ReflectionORMProperty extends ReflectionPropertyAnnotated {
	...
	public function getValue($object = null) {
		...
		if ($this->parentProperty) {
			...
			return $this->rflctGetter ? $parentValue->{$this->rflctGetter->getName()}() : $parentValue->{$this->getName()};
		} else {
			if ($this->isStatic()) return $object::$this->getName();
			return $this->rflctGetter ? $object->{$this->rflctGetter->getName()}() : $object->{$this->getName()};
		}
	}
	...
	public function setValue($object, $value = null) {
		...
		if ($this->parentProperty) {
			...
			return $this->rflctSetter ? $parentValue->{$this->rflctSetter->getName()}($value) : $parentValue->{$this->getName()} = $value;
		} else {
			return $this->rflctSetter ? $object->{$this->rflctSetter->getName()}($value) : $object->{$this->getName()} = $value;
		}
	}

	/** Modifica o valor de uma anotacao
	 *
	 * @param string $name
