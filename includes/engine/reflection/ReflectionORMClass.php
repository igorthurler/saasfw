...
class ReflectionORMClass extends ReflectionClassAnnotated {
    ...
    */
    public function ReflectionORMClass($argument) {
        ...
        //Verifica se esta anotada como uma entidade, senao dispara uma exception
        ...
        foreach ( (isset($parentProperty) ? $this->getORMProperties($parentProperty) : $this->getORMProperties())  as $property) {
            //Pulamos os transientes (nao persistentes)
            if ($property->isTransient()) continue;
            if ($property->isIndex()) {
                //Indice alfanumerico?
                if (in_array(trim($property->getType()),array('string','date')))
                    if ($property->getGenerationStrategy() == GenerationType::AUTO || $property->getGenerationStrategy() == GenerationType::MAX)
                    throw new ORMException("[ORM] A proriedade [".$this->getName().".".$property->getName()."] é um indice alfanumerico e nao deve possuir uma estrategia de geracao automatica (MAX ou AUTO).");
                	
                //Indice FK
                if (class_exists($property->getType())  && in_array( $property->getType(), get_declared_classes() )) {

                    $fkRf = new ReflectionORMClass($property->getType(),null,$property);
                    $fkIndexes = $fkRf->getIndexes($property);
            ...
            if ($property->isJoined() && $property->getDeclaringClass()->getName() == $this->getName()) {
                $this->joins[] = $property;
    ...
    public function getIndexes() {
        ...
        } else if ($this->getParentClass() && $this->getInheritanceStrategy() == InheritanceType::TABLE_PER_CLASS) {
            if ($this->hasAnnotation("JoinColumn")) {
                $parentIndexes = $this->getParentORMClass()->getIndexes();
                $joinColumns = ( (is_array($this->getAnnotation("JoinColumn")->getName())) ? $this->getAnnotation("JoinColumn")->getName() : array($this->getAnnotation("JoinColumn")->getName()) );
                if (sizeof($parentIndexes) != sizeof($joinColumns)) throw new ORMException("[ORM] A quantidade de colunas definidas em JoinColumn da classe ".$this->getName()." difere da classe pai ".$this->getParentORMClass()->getName());
                for ($i = 0; $i < sizeof($parentIndexes); $i++) {
                    $parentIndexes[$i] = new ReflectionORMProperty($parentIndexes[$i]->getDeclaringClass()->getName(), $parentIndexes[$i]->getName(), null, (isset($parentProperty) ? $parentProperty : null));
                    $parentIndexes[$i]->__overrideAnnotation("Column",new ColumnAnnotation($joinColumns[$i]));
                }
                return $parentIndexes;
            } else {
                return $this->getParentORMClass()->getIndexes();
            }
        } else {
            return $this->indexes;
        }
    ...
    public function hasORMProperty($name) {
        $rf = $this;
