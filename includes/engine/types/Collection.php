...
class Collection extends ArrayList {
	...
	public function &remove($object) {
		...
		if ($this->contains($object,$content)) {
	...
	/** Verifica se o objeto esta contido na colecao
	 * @param object $object Objeto para procurar na colecao
	 * @param mixed $found Item encontrado
	 * @return boolean
	 */ 
	public function contains(&$object, &$found = null) {
		
		if (is_object($object)) {
			if ($object instanceof IEntityContainer) {
				$rf = new ReflectionClassAnnotated($object->getSubject());
				$objOID = EntityCacher::makeOIDString($object->getSubject());
				
			} else {
				$rf = new ReflectionClassAnnotated($object);
				$objOID = EntityCacher::makeOIDString($object);
			}
			if ($rf->hasAnnotation("Entity") && $objOID) {
				foreach ($this->a as &$e) {
					if (is_object($e)) {
						if ($e instanceof IEntityContainer) {
							$current = $e->getSubject();
						} else {
							$current = $e;
						}
						if (EntityCacher::makeOIDString($current) == $objOID) {
							$found = $e;
							return true;
						}
					}
