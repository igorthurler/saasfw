...
import('engine.db.orm.ORMRequest');
import('engine.db.orm.ORMBuilder');
import('engine.db.orm.ORMLoader');
import('engine.db.orm.ORMLoaderBuilder');
import('engine.db.orm.ORMPersister');
import('engine.db.orm.ORMPersisterBuilder');
import('engine.db.orm.ORMDeleter');
import('engine.db.orm.ORMDeleterBuilder');
import('engine.db.filter.EntityFilter');
        ...
        try {
            ...
            foreach ($arr as $en) {
                if ($className != get_class($en))
                    throw new Exception("[DAO] Somente um array contendo instancias da mesma classe pode ser fornecido.");
            }
            ...
            //Old
            if ($entityFilter) {
                $builder = new ORMLoaderBuilder(true,$entityFilter);
                $builder->build($className);
                $loader = $builder->getRoot();
            } else {
                if(defined('ENGINE_NO_BUILDER_CACHE')) {
                    $builder = new ORMLoaderBuilder(true);
                    $builder->build($className);
                    $loader = $builder->getRoot();
                } else {
                    $loader = EntityManager::getLoader($className);
                }
            }
    ...
    public function save(&$entity) {
        try {
            ...
            if(defined('ENGINE_NO_BUILDER_CACHE')) {
                ...
                $persister = EntityManager::getPersister($className);
            }
    ...
    public function delete(&$entity) {
        try {
            ...
            if(defined('ENGINE_NO_BUILDER_CACHE')) {
                ...
                $deleter = EntityManager::getDeleter($className);
            }
