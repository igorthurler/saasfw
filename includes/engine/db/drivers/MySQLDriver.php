		...
		if(!$aux) {
			...
			if (count($matches['indexIdx']) > 0) {
				...
				if (count($matches['table']) > 0) {
					...
					$e = new ConstraintException("[MySQLDriver] A constraint UNIQUE ".$table.".".$indexName." foi violada.");
			...
			if (in_array(substr($sql, 0, 6), array("INSERT","UPDATE","DELETE"))) {
				...
				*/
				...
				$strInfo = mysql_info($this->getCon());
				if ($strInfo) {
				...
				*/
				...
				if (($mysql_affected_rows == 0)&&(mysql_affected_rows($this->getCon()) == 0)) {
					...
					throw new NoRecordsAffectedException("[MySQLDriver] A operacao nao afetou nenhum registro no banco de dados. ". ($debugData != "" ? "[".$debugData."]" : ""));
				}
			}
			$this->setRes($aux);
			return $aux;
