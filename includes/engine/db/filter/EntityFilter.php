...
import('engine.db.filter.FilterParameter');
...
class EntityFilter {
    ...
    */
    ...
    public function getFetchMode($path) {
        if (empty($path)) throw new Exception("Caminho nao fornecido");
    ...
    public function getSqlString() {
        ...
        if ($this->getClass()->getParentClass() && $this->getClass()->getInheritanceStrategy() == InheritanceType::TABLE_PER_CLASS) {
            ...
            for ($j = $i; $j>=0; $j--) {
                ...
                $parentIndex = $parentClass[$j]->getIndexes();
                $currentIndex = $currentClass[$j]->getIndexes();

                $join = " INNER JOIN ".$driver->formatTable($currentClass[$j]->getTableName())." ".$driver->formatTable($this->getAlias().$currentClass[$j]->getTableName())." ON (";

                if ($currentClass[$j]->hasAnnotation("CompositeColumnAnnotation")) {
                    $k = 0;
                    foreach ($currentClass[$j]->getAnnotation("CompositeColumnAnnotation")->getColumns() as $currentIndex) {
                        $on[] = $driver->formatTable($this->getAlias().$currentClass[$j]->getTableName()).".".$driver->formatField($currentIndex->getName())." = ".$driver->formatTable($this->getAlias().$parentClass[$j]->getTableName()).".".$driver->formatField($parentIndex[$k]->getColumnName());
                        $k++;
                    }
                    //Padrao
                } else {
                    for ($k = 0; $k < sizeof($currentIndex); $k++) {
                        $on[] = $driver->formatTable($this->getAlias().$currentClass[$j]->getTableName()).".".$driver->formatField($currentIndex[$k]->getColumnName())." = ".$driver->formatTable($this->getAlias().$parentClass[$j]->getTableName()).".".$driver->formatField($parentIndex[$k]->getColumnName());
                    }
                }
            ...
            $alias = $this->getAlias().$this->getClass()->getTableName();
        ...
        foreach ( array_keys($this->aliases) as $refrAlias ) {
            ...
            if ($property->isOneToOne()) {
                ...
                $joinedTable = $driver->formatTable($refrClass->getTableName());

                $join = $joinTypeStr.$joinedTable." ".$driver->formatTable($refrAlias)." ON (";
                if ($property->getMappedBy()) { //MappedBy
                    $thisIndexes = $thisClass->getIndexes();
                    $refrProperty = $refrClass->getORMProperty($property->getMappedBy());
                    ...
                    for ($i = 0; $i < sizeof($refrColumns); $i++) {
                        $on[] = " ".$thisAlias.".".$driver->formatField($thisIndexes[$i]->getColumnName())." = ".$driver->formatTable($refrAlias).".".$driver->formatField($refrColumns[$i]->getName());
                    }
                } else { //MappedIn
                    ...
                    for ($i = 0; $i < sizeof($thisColumns); $i++) {
                        $on[] = " ".$thisAlias.".".$driver->formatField($thisColumns[$i]->getName())." = ".$driver->formatTable($refrAlias).".".$driver->formatField($refrIndexes[$i]->getColumnName());
                    }
                }
                $joins[] = $join.implode(" AND ",$on).")";
            } else
            ...
            $join = $driver->formatTable($joinTypeStr.$joinedTable)." ".$driver->formatTable($refrAlias)." ON (";
            if ($property->getMappedBy()) { //MappedBy
                $thisIndexes = $thisClass->getIndexes();
                $refrProperty = $thisClass->getORMProperty($property->getMappedBy());
                if ($refrProperty->isComposite()) {
            ...
            $join = " LEFT JOIN ".$driver->formatTable($this->getRflctORM()->getTableName())." ".$driver->formatTable($colAlias)." ON (";
            $invJoin = " LEFT JOIN ".$driver->formatTable($refrTable)." ".$driver->formatTable($refrAlias)." ON (";
            $onJoin = array();
            $onInverse = array();
            for ($i = 0; $i < sizeof($joinColumns); $i++) {
                $onJoin[] = " ".$thisAlias.".".$driver->formatField($joinColumns[$i])." = ".$driver->formatTable($colAlias).".".$driver->formatField($thisIndexes[$i]->getColumnName());
                $onInverse[] = " ".$colAlias.".".$driver->formatField($inverseJoinColumns[$i])." = ".$driver->formatTable($refrAlias).".".$driver->formatField($refrIndexes[$i]->getColumnName());
            }
        ...
        foreach ($thisIndexes as $index) {
            $fields[] = $driver->formatTable($alias).".".$driver->formatField($index->getColumnName())." AS ".$driver->formatField($index->getName());
        }

        //Groupings
        foreach ($this->groupBy as $grouping) {
            $fields[] = $grouping->toSql($this);
        }

        $sql = $select.implode(",",$fields)
        ." FROM ".$driver->formatTable($tableName)." ".$driver->formatTable($alias)
        .implode(" \n",$joins)
        ." WHERE ";
        if (count($ql) > 0) {
            $sql .= implode(" AND ",$ql);
        ...
        if (count($this->groupBy) > 0) {
            ...
            //Indices primeiro
            foreach ($thisIndexes as $index) {
                $grp[] = $driver->formatTable($alias).".".$driver->formatField($index->getColumnName());
            }
            //Groupings
    ...
    public function getUnique() {
        ...
        if (count($data) == 1) {
            ...
            throw new Exception("[Runtime] Mais de um resultado encontrado utilizando o filtro.");
        }
        return null;
    }
    ...
    public function addOrderBy(FilterOrderBy $order) {
        if (sizeof($this->orderBy) > 0 && $order->getOrder() == "RAND") throw new ORMException("[ORM] Order By Rand deve ser exclusivo");
        foreach ($this->orderBy as $addedOrder) {
            if ($addedOrder->getOrder() == "RAND") throw new ORMException("[ORM] Order By Rand deve ser exclusivo");
        }
        $this->orderBy[] = $order;
        return $this;
    }
    ...
    /** Retorna o numero de registros que o filtro atual obtem
     * @return int
     */
    public function getCount() {
        try {
            ...
            $count = $this->getDAO()->getDriver()->getCount($sql);
            if (!$this->getDAO()->isOngoingTransaction()) {
                $this->getDAO()->getDriver()->disconnect();
            }
            return $count;
    ...
    //Refactor
    private function navigateInheritance(ReflectionORMClass $class) {
        ...
        if ($class->getParentClass()) {
            $this->navigateInheritance($class->getParentORMClass());
            $ORM->addAssociation(new ORMAssociation($parentORM,$ORM,"super",JoinType::INNER_JOIN));
        }
        if ($class->getInheritanceStrategy() == InheritanceType::TABLE_PER_CLASS) {
            //adiciona items
        }

        if ($class->getParentClass() && $class->getInheritanceStrategy() == InheritanceType::TABLE_PER_CLASS) {

            $this->navigateInheritance($class->getParentORMClass());

            $i = 0;
            $parent = $class->getParentORMClass();
            ...
            $alias = $this->getAlias().$class->getTableName();
            $thisIndexes = $class->getIndexes();
        }
    }
    //Refactor
}
