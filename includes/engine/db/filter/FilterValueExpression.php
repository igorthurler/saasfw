    ...
    public function toSql(EntityFilter $entityFilter) {
        ...
        if (count($splitPath) > 1 ) {
            ...
            if ($entityFilter->getClass()->getParentClass()) {
                ...
                if (!$has) throw new Exception("A classe ".$this->class." nao possui a propriedade ".$this->getProperty());
            } else {
                $class = $entityFilter->getClass()->getName();
                $alias = $entityFilter->getAlias().$entityFilter->getClass()->getTableName();
            }
        }
        ...
        $sql = $entityFilter->getDAO()->getDriver()->formatTable($alias).".".$entityFilter->getDAO()->getDriver()->formatField($refrProperty->getColumnName())
        .$this->getOperand();
        ...
        if (is_array($value)) {
            foreach($value as $key => &$val) {
                if (is_null($value)) {
                    ...
                    $val = $entityFilter->getDAO()->getDriver()->formatValue(
            ...
            if (is_null($value)) {
                ...
                $sql .= $entityFilter->getDAO()->getDriver()->formatValue(
                        $refrProperty->getType(),
                        $value
                );
            }
